...
## 流程概述
用户通过 API 创建进化任务，后台守护线程执行遗传算法搜索最优策略，通过 WebSocket 实时推送代际进度，最终将冠军策略保存到策略库。
...
| EvolutionRunner._execute_task | DB task_row | 加载数据 (mtf_loader)、创建 EvolutionEngine、注册回调 | 后台执行上下文 |
