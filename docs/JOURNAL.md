# doc-rebuild 日志

- [2026-04-23] [doc-rebuild] 启动首次模式，扫描工程目录结构
- [2026-04-23] [doc-rebuild] 产出模块地图，写入 INDEX.md（后端 12 模块 + 前端 7 模块）
- [2026-04-23] [doc-rebuild] 完成模块 B4 策略 DNA，建立 backend/architecture.md
- [2026-04-23] [doc-rebuild] 完成模块 B3 技术指标与信号
- [2026-04-23] [doc-rebuild] 完成模块 B5 回测引擎
- [2026-04-23] [doc-rebuild] 完成模块 B6 进化引擎
- [2026-04-23] [doc-rebuild] 完成模块 B2 数据存储与加载
- [2026-04-23] [doc-rebuild] 完成模块 B1 API 入口与路由
- [2026-04-23] [doc-rebuild] 完成模块 F2 进化中心 UI
- [2026-04-23] [doc-rebuild] 完成模块 F3 策略实验室 UI
- [2026-04-23] [doc-rebuild] 完成模块 F4 图表组件
- [2026-04-23] [doc-rebuild] 完成模块 B7 评分系统
- [2026-04-23] [doc-rebuild] 完成模块 B8 验证与场景
- [2026-04-23] [doc-rebuild] 完成模块 B9 规则发现
- [2026-04-23] [doc-rebuild] 完成模块 B10 持久化
- [2026-04-23] [doc-rebuild] 完成模块 B11 日志
- [2026-04-23] [doc-rebuild] 完成模块 B12 可视化
- [2026-04-23] [doc-rebuild] 后端 12 模块全部完成
- [2026-04-25] [doc-rebuild] 启动全量文档更新（D0-D6），覆盖 MTF 引擎新增 + 前端待处理模块
- [2026-04-25] [doc-rebuild] D0: 全量扫描完成，识别后端 10 文件 + 前端 4 模块的文档差距
- [2026-04-25] [doc-rebuild] D1: 新增 mtf-engine.md（MTF 共振引擎模块文档）
- [2026-04-25] [doc-rebuild] D2: 更新 strategy-dna.md（derive_role、MTF 控制参数、direction=mixed、SignalSet 新字段）
- [2026-04-25] [doc-rebuild] D3: 更新 evolution-engine.md（3 MTF 变异算子、diversity MTF 签名、population MTF 层生成）
- [2026-04-25] [doc-rebuild] D4: 更新 backtest-engine.md（双向交易、from_order_func）和 data-storage.md（mtf_loader 日志）
- [2026-04-25] [doc-rebuild] D5: 新增前端 4 模块文档（pages-routes/services/state-management/types-utils）
- [2026-04-25] [doc-rebuild] D6: 更新 backend/architecture.md 和 INDEX.md，全量文档更新完成
- [2026-04-25] [doc-rebuild] D7: 全量深层文档重写 -- 19 个模块文档补充 6 维度（逻辑/链路/机制/接口/参数/约定），基于源码逐行分析
- [2026-04-30] [doc-rebuild] 按需刷新: B7 评分系统（v1.0 修回撤 bug + v2.0 精简维度/新模板 + Alpha 维度）、B6 进化引擎（种群排序暴露修复 + 模板偏置）、B5 回测引擎（benchmark_close + BacktestResult 新字段）、architecture.md + INDEX.md 同步更新
- [2026-04-30] [doc-rebuild] v0.15-0.17 文档更新: 新建 B13 模拟交易系统模块文档(trading-system.md)、更新 backend/architecture.md、更新 F5 服务层文档(services.md)、新建 F8 模拟交易页面文档(trading-page.md)、更新 INDEX.md
- [2026-05-01] [doc-rebuild] v0.15-0.17 修复后文档同步: B13 trading-system.md 全面重写(滑点/统计恢复/信号shift/checkpoint/dna校验/分页total)、F8 trading-page.md 全面重写(策略库创建链路/route state 自动创建/空状态导航)、architecture.md 工作流4+设计约束更新
